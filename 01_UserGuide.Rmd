...
### `AnnualPeriodicity` (now Projection Interval; matrix form)
...
### `MatrixPopulation` (Now Population name; population form)
...
### `MatrixComposite` (Now Composition; matrix form)
...
### `MatrixTreatment` (now Treatment; matrix form)
...
### `MatrixCaptivity` (now captivity; matrix form)
...
### `MatrixStartSeason` (now Seasonal PLUS 2 more fields: Start Season and End Seaon; Matrix form)

**Definition:** First season of study. 

Possible values for Seasonal:

* `No`
* `Yes`

Possible values for Start Season:
...
### `MatrixStartMonth` (Start Month; matrix form)
...
### `MatrixEndYear` (End Year; matrix form)
...
### `MatrixEndSeason` (End Season; matrix form)
...
### `MatrixEndMonth` (End Month; matrix form)
...
### `MatrixDimension` (Dimensions; matrix form)
...
### `SurvivalIssue` (This is not a field, it needs to be calculated from the survival matrix. It is the summation of each column in the survival matrix. If a column sums to 1, then there’s an issue)
...
### `MatrixClassNumber` (has to be calculated from matrix class organized. The number of classes in a matrix)
...
**Description:** The version number of the database (Major_Minor_Patch). e.g. `3.1.0`. Numbering follows semantic versioning (www.semver.org).
